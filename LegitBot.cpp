...
{
	hitboxPosition_ = target->GetBonePosition(hitboxID, Interfaces::Globals->curtime, true, false, nullptr);
	...
	{
		if (!AimbotMultiPointChk.Checked || (AimbotMultiPointPrioritiesOnlyChk.Checked && !AllPlayers[target->index].PersistentData.PriorityTarget))
			return false;
...
bool Legitbot::GetTargetHitboxDynamic(CBaseEntity *localPlayer, CBaseEntity *target, int hitboxID)
{
	...
	for (auto &hitbox : hitboxes)
	{
		...
		return false;
#else
		temp = target->GetBonePosition(hitbox, Interfaces::Globals->curtime, true, false, nullptr);
#endif
