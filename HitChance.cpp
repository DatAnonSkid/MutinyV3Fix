...
void __stdcall Parallel_HitChance(HC_Vars* args)
{
	while (1)
	{
		...
		while (i < args->maxseeds)
		{
			...
			//Interfaces::DebugOverlay->AddLineOverlay(LocalEyePos, TraceEnd, 255, 0, 0, true, 0.1f);
...
bool BulletWillHit(CBaseCombatWeapon* Weapon, int buttons, CBasePlayer* pTargetEntity, QAngle viewangles, Vector LocalEyePos, Vector *pos, unsigned int TargetHitGroup)
{
	...
	if (Weapon)
	{
		...
		//Interfaces::DebugOverlay->AddLineOverlay(LocalEyePos, EndPoint, 255, 0, 0, true, 0.1f);
		...
		{
			CustomPlayer *pCPlayer = &AllPlayers[pTargetEntity->index];
			StoredNetvars *pBacktrackTick = pCPlayer->pBacktrackedTick;
			bool bCheckBodyYawIndex = pCPlayer->PersistentData.ShouldResolve();

			if (!pBacktrackTick)
			{
				pTargetEntity->SetupBonesRebuiltSimple(boneMatrixes, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, /*GetSimulationTime()*/ Interfaces::Globals->curtime);
				destBoneMatrixes = boneMatrixes;
			}
			else if (pBacktrackTick->bCachedBones && (!bCheckBodyYawIndex || pCPlayer->PersistentData.correctedbodyyawindex == pBacktrackTick->corrected_body_yaw_index))
			{
				destBoneMatrixes = pBacktrackTick->CachedBoneMatrixes;
				if (!pCPlayer->bUsedCachedBonesThisTick)
				{
					CThreadFastMutex* pBoneSetupLock = pTargetEntity->GetBoneSetupLock();
					if (!pBoneSetupLock->TryLock())
					{
						pBoneSetupLock->Lock();
					}
					CBoneAccessor *accessor = pTargetEntity->GetBoneAccessor();
					CUtlVectorSimple *CachedBones = pTargetEntity->GetCachedBoneData();
					...
					//accessor->SetReadableBones(pBacktrackTick->iReadableBones);
					//accessor->SetWritableBones(pBacktrackTick->iWritableBones);
					CachedBones->count = pBacktrackTick->iCachedBonesCount;
					memcpy(CachedBones->Base(), pBacktrackTick->CachedBoneMatrixes, sizeof(matrix3x4_t) * MAXSTUDIOBONES);
			...
			{
				pTargetEntity->SetupBonesRebuiltSimple(pBacktrackTick->CachedBoneMatrixes, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, /*GetSimulationTime()*/ Interfaces::Globals->curtime);
				destBoneMatrixes = pBacktrackTick->CachedBoneMatrixes;
				pBacktrackTick->bCachedBones;
				pBacktrackTick->iTickCachedBones = Interfaces::Globals->tickcount;
				pBacktrackTick->corrected_body_yaw_index = pCPlayer->PersistentData.correctedbodyyawindex;
			}
		}
		else
#endif
		{
			...
			if (pTargetEntity->IsPlayer())
			{
				...
				if (pCPlayer->bAlreadyCachedBonesThisTick)
					pTargetEntity->SetupBones(destBoneMatrixes, MAXSTUDIOBONES, BONE_USED_BY_HITBOX, Interfaces::Globals->curtime);
				else
				{
					pTargetEntity->InvalidateBoneCache();
					pTargetEntity->SetLastOcclusionCheckFlags(0);
					pTargetEntity->SetLastOcclusionCheckFrameCount(Interfaces::Globals->framecount);
					pTargetEntity->SetupBones(destBoneMatrixes, MAXSTUDIOBONES, BONE_USED_BY_ANYTHING, Interfaces::Globals->curtime);
		...
		for (int i = 0; i < set->numhitboxes; i++)
		{
			...
			{
				Vector vMin, vMax;
				VectorTransformZ(pbox->bbmin, destBoneMatrixes[pbox->bone], vMin);
				VectorTransformZ(pbox->bbmax, destBoneMatrixes[pbox->bone], vMax);
			...
			{
				m_cOBBs.push_back(COBB(pbox->bbmin, pbox->bbmax, &destBoneMatrixes[pbox->bone], i, pbox->group));
			}
		}
		END_PROFILING(setup)
#endif
...
#if 1
		int maxallowedmiss = 256 - totalseedsneeded;
		int i = 0;
		...
		while (i < 256)
		{
			...
			if (SeedsHit >= totalseedsneeded || i - SeedsHit > maxallowedmiss)
				break;
		}
#else
