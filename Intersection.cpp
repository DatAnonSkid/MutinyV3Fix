...
void SetupCapsule(const Vector& vecMin, const Vector& vecMax, float flRadius, int hitbox, int hitgroup, std::vector<CSphere>&m_cSpheres)
{
	...
	m_cSpheres.push_back(CSphere{ vecMin, flRadius, hitbox, hitgroup });

	for (size_t i = 1; i < std::floor(vecMin.Dist(vecMax)); ++i) {
		m_cSpheres.push_back(CSphere{ vecMin + vecDelta * static_cast< float >(i), flRadius, hitbox, hitgroup });
	}

	m_cSpheres.push_back(CSphere{ vecMax, flRadius, hitbox, hitgroup });
}
