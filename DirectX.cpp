...
long __stdcall Hooked_EndScene(IDirect3DDevice9* pDevice)
{
	...
	if (!Framework->Menu)
	{
		...
		Framework->Menu->Config = MutinyFrame::CWebConfig("api.mutiny.pw", "/auth.php", Framework->Heartbeat->hUserAgent.c_str(), Framework->Heartbeat->hKey.c_str(), Framework->Heartbeat->hIV.c_str());
