...
public:
	...
	ModifiableUserCmd(CUserCmd* newcmd){ cmd = newcmd; }

	~ModifiableUserCmd() {}

	void Reset(CUserCmd* newcmd){ cmd = newcmd; }
	void SetIsFakeAngle(bool fake) { real = fake ? false : true; }
	bool IsFakeAngle() { return !real; }
	bool IsRealAngle() { return real; }
	bool bSendPacket; //Should we send a cmd this tick
	int iCmdsChoked; //Number of cmds choked since last send
private:
	bool real;
};
