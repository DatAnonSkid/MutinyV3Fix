...
bool AutoBone::ShouldBodyAim(CBaseCombatWeapon *weapon, CustomPlayer* pCAPlayer, StoredNetvars *targettick, StoredNetvars *backtracktick)
{
	if (AimbotAimTorsoChk.Checked || AimbotAimArmsHandsChk.Checked || AimbotAimLegsFeetChk.Checked)
	{
		CBaseEntity *Entity = pCAPlayer->BaseEntity;

		if (AutoPelvisChk.Checked)
		{
			if (pCPlayer->TicksChoked > 0)
			{
				if ((targettick->FireAtPelvis && !pCPlayer->bIsHistoryBacktracked) || !(Entity->GetFlags() & FL_ONGROUND)) //|| AirJumping)
					return true;
			}
		}

		if (BodyAimIfNoBacktrackTickChk.Checked && !backtracktick)
		{
			return true;
		}

		if (BodyAimAfterXMissesTxt.iValue && pCPlayer->PersistentData.ShotsMissed >= BodyAimAfterXMissesTxt.iValue)
			return true;

		float BaimBelowThisHealth = weapon->IsPistol(NULL) ? (BodyAimBelowThisHealthTxt.flValue * 0.25f) : BodyAimBelowThisHealthTxt.flValue;
		if (pCPlayer->BaseEntity->GetHealth() < BaimBelowThisHealth)
			return true;
...
bool AutoBone::Run_AutoBone(CBaseCombatWeapon* weapon, CustomPlayer* pCAPlayer, QAngle *LocalEyeAngles, Vector LocalEyePos, int& BestBone, Vector& BestBonePos, bool& RanHitChance, bool& CanTraceALine)
{
	...
	if (weapon)
	{
		...
		//Shit fix for now until hitchance + RCS is implemented correctly
		if (IsAllowedToNoRecoil(pWeapon) && pWeapon)
		{
			QAngle vecPunch = LocalPlayer.Entity->GetPunch();
			angAimPunchToSubtract.x = (vecPunch.x * 2.0f) * (NoRecoilYPercentTxt.flValue * 0.01f);
			angAimPunchToSubtract.y = (vecPunch.y * 2.0f) * (NoRecoilXPercentTxt.flValue * 0.01f);
		}
		...
		targettick = backtracktick ? backtracktick : pCPlayer->PersistentData.m_PlayerRecords.size() > 0 ? pCPlayer->PersistentData.m_PlayerRecords.front() : nullptr;
		...
		pCPlayer->FireAtPelvis = ShouldBodyAim(weapon, pCPlayer, targettick, backtracktick);
		...
		if (DetectPeekersChk.Checked/* && !pCPlayer->FireAtPelvis*/)
		{
			...
			{																					//while this is true ^ if they slow peak the corner so that their head isn't shown you want to hit their legs if they are slow
				if (!pCPlayer->FireAtPelvis && HeadshotOnlyPeekersChk.Checked && AimbotAimHeadNeckChk.Checked)
				{
					HeadOnly = true;
				}
		...
		{
			//Regular hitbox aim
			if (AimbotAimHeadNeckChk.Checked && !pCPlayer->PersistentData.DontShootAtHeadNeck && !pCPlayer->FireAtPelvis
				&& !AimbotAimTorsoChk.Checked && !AimbotAimLegsFeetChk.Checked && !AimbotAimArmsHandsChk.Checked)
				HeadOnly = true;
			...
			if (!pCPlayer->FireAtPelvis && !pCPlayer->PersistentData.DontShootAtHeadNeck && AimbotAimHeadNeckChk.Checked)
			{
				bool Lethal = false;
				bool DealsMinimumDamage = false;
		...
		else if ((BacktrackWhenVisibleChk.Checked || !CanTraceALine) && !pSilentChk.Checked && pCPlayer->PersistentData.ShouldResolve())
		{
			//Attempt to backtrack them further to a spot that was hittable
			StoredNetvars *HistoryTick = pCPlayer->GetBestHistoryTick();
			if (HistoryTick && HistoryTick != pCPlayer->pBacktrackedTick)
			{
				CanTraceALine = true;

				gLagCompensation.CM_BacktrackPlayer(pCPlayer, LocalPlayer.Entity, pCPlayer->BaseEntity, HistoryTick);
				pCPlayer->BaseEntity->SetAbsAngles(HistoryTick->absangles);
				pCPlayer->pBacktrackedTick = HistoryTick;
				pCPlayer->pLastBacktrackedTick = HistoryTick;
				pCPlayer->bIsHistoryBacktracked = true;
				BestBone = -1;
				float flBestDamage = 0.0f;

				if (HistoryTick->bHeadIsVisible && AimbotAimHeadNeckChk.Checked)
				{
					//Todo: 1 function for this, just use pointers
					const int count = (sizeof(HITBOX_GROUP_HEAD) / sizeof(HITBOX_GROUP_HEAD[0]));
					for (int hbox = HITBOX_GROUP_HEAD[0]; hbox < count; hbox++)
					{
						SavedHitboxPos* savedhitbox = &HistoryTick->BestHitboxPos[hbox];
						if (savedhitbox->bsaved)
						{
							float flSavedDmg = savedhitbox->damage;
							if (flSavedDmg > flBestDamage)
							{
								flBestDamage = flSavedDmg;
								BestBone = hbox;
								BestBonePos = savedhitbox->origin;
							}
						}
					}
				}
				if (HistoryTick->bBodyIsVisible)
				{
					if (AimbotAimTorsoChk.Checked)
					{
						//Todo: 1 function for this, just use pointers
						const int count = (sizeof(HITBOX_GROUP_TORSO) / sizeof(HITBOX_GROUP_TORSO[0]));
						for (int hbox = HITBOX_GROUP_TORSO[0]; hbox < count; hbox++)
						{
							SavedHitboxPos* savedhitbox = &HistoryTick->BestHitboxPos[hbox];
							if (savedhitbox->bsaved)
							{
					...
					if (AimbotAimArmsHandsChk.Checked)
					{
						...
						for (int hbox = HITBOX_GROUP_UPPERLIMBS[0]; hbox < count; hbox++)
						{
							SavedHitboxPos* savedhitbox = &HistoryTick->BestHitboxPos[hbox];
							if (savedhitbox->bsaved)
							{
					...
					if (AimbotAimLegsFeetChk.Checked)
					{
						...
						for (int hbox = HITBOX_GROUP_LOWERLIMBS[0]; hbox < count; hbox++)
						{
							SavedHitboxPos* savedhitbox = &HistoryTick->BestHitboxPos[hbox];
							if (savedhitbox->bsaved)
							{
				...
				if (BestBone != -1)
				{
					if (BacktrackESPChk.Checked)
					{
						//pCPlayer->BaseEntity->DrawHitboxesFromCache(ColorRGBA(0, 255, 0, 255), 0.5f, HistoryTick->CachedBoneMatrixes);
						pCPlayer->BaseEntity->DrawHitboxes(ColorRGBA(0, 255, 0, 255), 0.5f);
					}
					gTriggerbot.SetAllowedToFire(true);
					RanHitChance = false;
...
bool AutoBone::AutoBone_HeadOnly()
{
	...
	StoredNetvars *backtracktick = pCPlayer->pBacktrackedTick;
	if (backtracktick)
	{
		backtracktick->bRanHitScan = true;
	}

	PotentialHitboxPos = pPlayer->GetBonePosition(PotentialHitbox, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
...
#endif
	{
		*pCanTraceALine = true;
		...
		if (backtracktick)
		{
			backtracktick->bHeadIsVisible = true;
			SaveBestHitboxPos(pCPlayer, backtracktick, PotentialHitbox, PotentialDamage, PotentialHitboxPos);
		}

		//Head is a valid target now. Instead of aiming at the existing bone, aim at the head now		

		if (OnlyHitchanceInTriggerbot || BulletWillHit(pWeapon, buttons, pPlayer, CalcAngle(*pLocalEyePos, PotentialHitboxPos) - angAimPunchToSubtract, *pLocalEyePos, &PotentialHitboxPos, HITGROUP_HEAD_FLAG))
		{
...
bool AutoBone::AutoBone_ForcePelvisChest(bool *FoundLethalHitbox, bool *DealsMinimumDamage)
{
...
#else
	PotentialHitboxPos = pPlayer->GetBonePosition(BodyAimHitboxAlreadyChecked, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
...
#endif
	{
		*pCanTraceALine = true;
		...
		if (backtracktick)
		{
			backtracktick->bBodyIsVisible = true;
			SaveBestHitboxPos(pCPlayer, backtracktick, PotentialHitbox, PotentialDamage, PotentialHitboxPos);
		}
		...
		if (OnlyHitchanceInTriggerbot || BulletWillHit(pWeapon, buttons, pPlayer, CalcAngle(*pLocalEyePos, PotentialHitboxPos) - angAimPunchToSubtract, *pLocalEyePos, &PotentialHitboxPos, HITGROUP_CHEST_FLAG | HITGROUP_STOMACH_FLAG))
		{
			if (!pCPlayer->FireAtPelvis)
			{
				if (PotentialDamage > BestDamage)
	...
	for (int i = 0; i < reps; i++)
	{
...
#endif
		{
			...
			*pCanTraceALine = true;
			...
			if (OnlyHitchanceInTriggerbot || BulletWillHit(pWeapon, buttons, pPlayer, CalcAngle(*pLocalEyePos, PotentialHitboxPos) - angAimPunchToSubtract, *pLocalEyePos, &PotentialHitboxPos, flags))
			{
				if (PotentialDamage > BestDamage)
				{
					FoundHitbox = true;
	...
	for (int i = 0; i < reps; i++)
	{
...
#endif
		{
			...
			*pCanTraceALine = true;
			...
			if (OnlyHitchanceInTriggerbot || BulletWillHit(pWeapon, buttons, pPlayer, CalcAngle(*pLocalEyePos, PotentialHitboxPos) - angAimPunchToSubtract, *pLocalEyePos, &PotentialHitboxPos, HITGROUP_LEFTLEG_FLAG | HITGROUP_RIGHTLEG_FLAG))
			{
				if (PotentialDamage > BestDamage)
				{
					FoundHitbox = true;
	...
	for (int i = 0; i < NUM_BONES; i++)
	{
...
#else
		Vector PotentialHitboxPos = pPlayer->GetBonePosition(PotentialHitbox, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
#endif
		trace_t tr;
		float PotentialDamage;
		bool Visible;
		...
		if (Visible)
		{
			*pCanTraceALine = true;
			...
			if (backtracktick)
			{
				if (PotentialHitbox == HITBOX_HEAD || PotentialHitbox == HITBOX_NECK)
					backtracktick->bHeadIsVisible = true;
				else
					backtracktick->bBodyIsVisible = true;
...
bool AutoBone::AutoBone_HsNoSpread()
{
	...
	float damage = 0;
	Vector vMin, vMax;
	Vector PotentialHitboxPos = pPlayer->GetBonePositionTotal(hitbox, BoneTargetTime, vMax, vMin, false, true, nullptr);
