...
#define MAX_NEAR_BODY_YAW_RESOLVER_MISSES 7
...
#define MAX_LINEARFAKE_MISSES 4
#define MAX_FAKESPINS_MISSES 4
#define MAX_RANDOMFAKE_MISSES 4
#define MAX_STATICFAKEDYNAMIC_MISSES 4
#define MAX_AVGLBYDELTA_MISSES 4
...
struct Personalize 
{
	...
	ResolveYawModes LastEstimateYawMode; //The last estimated yaw mode used, so in case blood fails to register a hit and lower body yaw doesn't update then fallback to previous modes
	...
	int correctednotfakeindex;
	int correctedresolvewhenshootingindex;
	int correctedlbybreakerindex;
	int correctedfakespinindex;
	int correctedlinearindex;
	int correctedrandomindex;
	int correctedfakedynamicindex;
	int correctedavglbyindex;
	int LastHitGroupShotAt;
	QAngle LastEyeAnglesShotAt;
	QAngle EyeAnglesShotAt;
	int ShotsMissed;
	int iHitGroupPlayerWasShotOnServer;
	bool bAwaitingBloodResolve; //Let the blood resolver know it can attempt resolving
	float flSimulationTimePlayerWasShot;
	float flSimulationTimePlayerWasShotOnClient;
	...
	{
		return !DoNotResolve && (ResolveYawMode != ResolveYawModes::NoYaw || ResolvePitchMode != ResolvePitchModes::NoPitch);
	}
};
