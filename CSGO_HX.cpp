...
#endif
{
	...
	if (!FoundCSGO)
	{
		...
		if (Time - starttime < 120.0f)
		{
			...
			if (!ClientHandle || !MatchmakingHandle || !VPhysicsHandle || !VSTDLIBHandle || !SHADERAPIDX9Handle || !DatacacheHandle || !Tier0Handle)
			{
				END_PROFILING(stat);
				return;
			}
...
char *valve001str = new char[9]{ 44, 27, 22, 12, 31, 74, 74, 75, 0 }; /*Valve001*/
...
void GetCSGORect()
{
	...
	DecStr(valve001str, 8);
	tWnd = FindWindowA(NULL, csgostr);
	char outbuf[MAX_PATH];
	if (!tWnd)
	{
		tWnd = FindWindowA(valve001str, classicstr);
	}
	EncStr(csgostr, 32);
	EncStr(classicstr, 33);
	EncStr(valve001str, 8);
