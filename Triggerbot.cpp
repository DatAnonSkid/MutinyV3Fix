	...
	if (WeaponID != WEAPON_C4)
	{
		...
		else if (!weapon->IsGrenade(WeaponID))
		{
			...
			if (!Jumping)
			{
				if (TargetPlayer)
				{
					if ((OnlyShootIfLagCompensatedChk.Checked && !AllPlayers[TargetPlayer->index].IsBreakingLagCompensation) || (OnlyShootIfHistoryBacktrackedChk.Checked && !AllPlayers[TargetPlayer->index].bIsHistoryBacktracked))
					{
						END_PROFILING(stat);
						return;
					}
						...
						if (pSilentChk.Checked)
						{
							...
							if (RanHitChance || BulletWillHit(weapon, buttons, TargetPlayer, angles, LocalEyePos, nullptr, MTargetting.CURRENT_HITCHANCE_FLAGS))
							{
								...
								//if (!gAntiGOTV.GOTVIsRecording(CurrentUserCmd.cmd->tick_count))
								{
									...
									CustomPlayer* pCAPlayer = &AllPlayers[ReadInt((uintptr_t)&TargetPlayer->index)];
									pCAPlayer->PersistentData.LastHitGroupShotAt = MTargetting.CURRENT_HITGROUP_AIMING_AT;
									pCAPlayer->PersistentData.LastEyeAnglesShotAt = pCAPlayer->PersistentData.EyeAnglesShotAt;
									pCAPlayer->PersistentData.EyeAnglesShotAt = TargetPlayer->GetEyeAngles();
						...
						else
						{
							...
							if (manual || (canFire && (RanHitChance || BulletWillHit(weapon, buttons, TargetPlayer, angles, LocalEyePos, nullptr, MTargetting.CURRENT_HITCHANCE_FLAGS))))
							{
								...
								if (!WillBreakAntiAim)
								{
									...
									pCAPlayer->ResolverFiredAtShootingAngles = FireWhenEnemiesShootChk.Checked && pCAPlayer->bBacktracked && pCAPlayer->LatestFiringTick;
								}
							}
						}
					}
				...
				else
				{
					...
					if (IsRevolver || WeaponCanFire(weapon))
					{
						...
						{
							float dmg;
							EntityHit = Autowall(nullptr, LocalPlayer.Entity->GetEyePosition(), &(angles + (LocalPlayer.Entity->GetPunch() * 2.0f)), weapon, &dmg, hitgroup, nullptr, false);
						...
						else
						{
							...
							AngleVectors(angles + (LocalPlayer.Entity->GetPunch() * 2.0f), &vecDir);
						...
						if (CanHit)
						{
							if ((OnlyShootIfLagCompensatedChk.Checked && !AllPlayers[EntityHit->index].IsBreakingLagCompensation) || (OnlyShootIfHistoryBacktrackedChk.Checked && !AllPlayers[EntityHit->index].bIsHistoryBacktracked))
							{
								END_PROFILING(stat);
								return;
							}
							...
							if (!IsRevolver && Time >= TimeToShoot)
							{
								...
								{
									//Triggerbot Firing
									pCAPlayer->PersistentData.LastHitGroupShotAt = hitgroup;
									pCAPlayer->PersistentData.LastEyeAnglesShotAt = pCAPlayer->PersistentData.EyeAnglesShotAt;
									pCAPlayer->PersistentData.EyeAnglesShotAt = EntityHit->GetEyeAngles();
									FireBullet(weapon, buttons, DontSetViewAngles, false);
									TimeToShoot = 0.0f;
