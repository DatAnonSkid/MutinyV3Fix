...
void AutoGrief(int& buttons, QAngle &angles, QAngle RealViewAngles, int PressingAimbotKey)
{
	...
	if (LastGriefingTargetID != INVALID_PLAYER)
	{
		...
		if (pGrief && !pGrief->GetDormant() && pGrief->GetAlive())
		{
			...
			CustomPlayer *pCPlayer = &AllPlayers[pGrief->index];
			GriefChestPos = pGrief->GetBonePosition(HITBOX_CHEST, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
			Vector targForward;
