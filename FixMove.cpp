...
char *frictionstr = new char[12]{ 9, 12, 37, 28, 8, 19, 25, 14, 19, 21, 20, 0 }; /*sv_friction*/
char *stopspeedstr = new char[13]{ 9, 12, 37, 9, 14, 21, 10, 9, 10, 31, 31, 30, 0 }; /*sv_stopspeed*/

bool Run_Fakewalk(CBaseEntity* AimTarget, QAngle& TargetAAAngles, int Direction)
{
	//if (!AimTarget)
	{
	...
	Vector velocity;
	int    revolutions{}, chokes;
	if (!sv_friction)
	{
		DecStr(frictionstr, 11);
		DecStr(stopspeedstr, 12);
		sv_friction = Interfaces::Cvar->FindVar(frictionstr);
		sv_stopspeed = Interfaces::Cvar->FindVar(stopspeedstr);
		EncStr(frictionstr, 11);
		EncStr(stopspeedstr, 12);
	}

	if (LocalPlayer.OriginalMovementValues.x > 0)
		CurrentUserCmd.cmd->forwardmove = 450.0f;
	else if (LocalPlayer.OriginalMovementValues.x < 0)
		CurrentUserCmd.cmd->forwardmove = -450.0f;

	if (LocalPlayer.OriginalMovementValues.y > 0)
		CurrentUserCmd.cmd->sidemove = 450.0f;
	else if (LocalPlayer.OriginalMovementValues.y < 0)
		CurrentUserCmd.cmd->sidemove = -450.0f;

	if (CurrentUserCmd.cmd->sidemove == 0.0f && CurrentUserCmd.cmd->forwardmove == 0.0f)
		return false;

	static auto simulate_accel = [&](Vector &vel) {
		float velocity_len, friction, stopspeed, delta;

		velocity_len = vel.Length();
		friction = sv_friction->GetFloat();
		stopspeed = sv_stopspeed->GetFloat();

		if (stopspeed <= velocity_len)
			stopspeed = velocity_len;

		if (velocity_len >= 0.1f) {
			// calculate delta.
			delta = fmaxf(0.f, velocity_len - ((stopspeed * friction) * Interfaces::Globals->interval_per_tick));

			// calculate out velocity.
			vel *= (delta / velocity_len);
		}
	};

	if (LocalPlayer.fakewalk && LocalPlayer.Entity->GetFlags() & FL_ONGROUND)
	{
		velocity = LocalPlayer.Entity->GetVelocity();

		DWORD ClientState = *(DWORD*)pClientState;
		chokes = *(DWORD*)(ClientState + chokedcommands);

		do {
			if (velocity.Length() < 0.1f)
				break;

			simulate_accel(velocity);

			++revolutions;
		} while (revolutions < 15); // max choke.

									// zero forwardmove and sidemove.
		if (revolutions > (7 - chokes) || !chokes)
			CurrentUserCmd.cmd->forwardmove = CurrentUserCmd.cmd->sidemove = 0.f;

		// set bSendPacket.
		//*g_cl.m_send_packet = !(chokes < 7);
	}

#if 0
	//Dylan's ghetto fake walk. Works but isn't pretty
	if (LocalPlayer.fakewalk)
	{
		switch (CurrentUserCmd.iCmdsChoked)
		{
		case 2:
		case 3:
			if (LocalPlayer.OriginalMovementValues.x > 0)
			...
			break;
		default:
			CurrentUserCmd.cmd->forwardmove = CurrentUserCmd.cmd->sidemove = 0.0f;
		}
	}
#endif

	if (CurrentUserCmd.cmd->forwardmove == 0.0f)
	{
		buttons &= ~IN_FORWARD;
		buttons &= ~IN_BACK;
	}
	else if (CurrentUserCmd.cmd->forwardmove > 0.0f)
	{
		buttons |= IN_FORWARD;
		buttons &= ~IN_BACK;
	}
	...
	else if (CurrentUserCmd.cmd->sidemove > 0.0f)
	{
		buttons |= IN_MOVERIGHT;
		buttons &= ~IN_MOVELEFT;
	}
	...
	if (animstate)
	{
		...
		float nextupdatetime = LocalPlayer.NextEstimatedLowerBodyUpdateTime;

		if (curtime < nextupdatetime)
			//angles.y += bswitch ? -90.0f : 90.0f;
		{
			if (CurrentUserCmd.IsRealAngle())
			{
				if (Direction == NONE || Direction == BACK)
					TargetAAAngles.y += 90.0f; //LocalPlayer.LowerBodyYaw + 180.0f;
				else if (Direction == LEFT)
					TargetAAAngles.y += 90.0f;
				else if (Direction == RIGHT)
					TargetAAAngles.y -= 90.0f;
				if (fabsf(ClampYr(TargetAAAngles.y - LocalPlayer.LowerBodyYaw)) < 40.0f)
					TargetAAAngles.y += RandomFloat(40.0f, 60.0f);
				if (fabsf(ClampYr(TargetAAAngles.y - LocalPlayer.LowerBodyYaw)) < 40.0f)
					TargetAAAngles.y += RandomFloat(40.0f, 60.0f);
			}
			else
			{
				if (Direction == NONE || Direction == BACK || Direction == LEFT)
					TargetAAAngles.y -= 90.0f;
				else
					TargetAAAngles.y += 90.0f;
			}
...
void Do_AntiAims(QAngle& TargetAAAngles, bool& LegitAAed, DWORD* FramePointer)
{
	...
	static ManualAA Direction = NONE;
	...
	for (int i = 0; i < 4; i++)
	{
		...
		{
			Direction = (ManualAA)i;
			break;
		}
		Key++;
	}

	if (LocalPlayer.fakewalk && Run_Fakewalk(LastTargetIndex == INVALID_PLAYER ? nullptr : AllPlayers[LastTargetIndex].BaseEntity, TargetAAAngles, Direction))
		return;
	...
	case ANTIAIM_TESTING:
		FixMove_Testing(TargetAAAngles, LocalPlayer.LastEyeAngles, LocalPlayer.CurrentEyeAngles, LastTargetIndex == INVALID_PLAYER ? nullptr : AllPlayers[LastTargetIndex].BaseEntity, FramePointer, buttons, Direction);
		break;
...
void MoveFix(QAngle angles, QAngle OriginalAngs, Vector Move)
{
	...
	AngleVectors(OriginalAngs, &viewForward, &viewRight, &viewUp);
	...
	CurrentUserCmd.cmd->forwardmove = DotProduct(viewForward * Move.x, aimForward) + DotProduct(viewRight * Move.y, aimForward) + DotProduct(viewUp * Move.z, aimForward);
	CurrentUserCmd.cmd->sidemove = DotProduct(viewForward * Move.x, aimRight) + DotProduct(viewRight * Move.y, aimRight) + DotProduct(viewUp * Move.z, aimRight);
#if 0
			...
			else
			{
				...
				else
				{
					...
					if (AimTarget)
					{
						...
						Vector lowerbodyheadpos = LocalPlayer.Entity->GetBonePosition(HITBOX_HEAD, /*LocalPlayer.Entity->GetSimulationTime()*/Interfaces::Globals->curtime, true, false, nullptr);
...
void FixMove_SideAntiAim(QAngle &angles, QAngle LastAngle, QAngle RealViewAngles, CBaseEntity* AimTarget, DWORD* FramePointer)
{
	...
	if (NumStreamedEnemies != 0)
	{
		for (int i = 0; i < NumStreamedPlayers; i++)
		{
			...
			if (pCPlayer->IsTarget && pCPlayer->PersistentData.ShouldResolve())
			{
...
float flNextGuessLowerBodyUPdateTime = 0.0f;

void FixMove_Testing(QAngle &angles, QAngle LastAngle, QAngle RealViewAngles, CBaseEntity* AimTarget, DWORD* FramePointer, int& buttons, int Direction)
{
#if 1
	float curtime = Interfaces::Globals->curtime;
	float nextupdatetime = LocalPlayer.NextEstimatedLowerBodyUpdateTime;
	...
	if (animstate->m_velocity > 0.1f)
	{
		...
		if (curtime < nextupdatetime)
		{
			...
			{
				if (Direction == NONE || Direction == BACK)
					angles.y += 180.0f; //LocalPlayer.LowerBodyYaw + 180.0f;
				else if (Direction == LEFT)
					angles.y += 90.0f;
				else if (Direction == RIGHT)
					angles.y -= 90.0f;
				if (fabsf(ClampYr(angles.y - LocalPlayer.LowerBodyYaw)) < 40.0f)
					angles.y += RandomFloat(40.0f, 60.0f);
			}
			else
			{
				if (Direction == NONE || Direction == BACK)
					angles.y = angles.y; //do nothing
				else if (Direction == LEFT)
					angles.y -= 90.0f;
				else
					angles.y += 90.0f;
	...
	else if (curtime < nextupdatetime)
		//angles.y += bswitch ? -90.0f : 90.0f;
	{
		if (CurrentUserCmd.IsRealAngle())
		{
			if (Direction == NONE || Direction == BACK)
				angles.y += 90.0f; //LocalPlayer.LowerBodyYaw + 180.0f;
			else if (Direction == LEFT)
				angles.y += 90.0f;
			else if (Direction == RIGHT)
				angles.y -= 90.0f;
	...
	float flServerTime = gLagCompensation.GetEstimatedServerTime();

	float flDelta = LocalPlayer.NextLowerBodyUpdateTime - flServerTime;

	float flFakeYaw = 90.0f;
	float flRealYaw = -90.0f;

	//static float flLastNextLowerBodyUpdateTime = LocalPlayer.NextLowerBodyUpdateTime;

	DWORD cl = GetServerClientEntity(1);
	CBaseEntity *pEnt = ServerClientToEntity(cl);
	float nextrealupdatetime = pEnt->GetNextLowerBodyyawUpdateTimeServer();

	//printf("nextreal %f\n", nextrealupdatetime);

	if (CurrentUserCmd.IsRealAngle())
	{
		//CurrentUserCmd.cmd->tick_count += 10;
		//CurrentUserCmd.cmd->command_number += 7 + CurrentUserCmd.cmd->tick_count % 2 ? 0 : 1;
	...
	{
		angles.y += 90.0f;
		CurrentUserCmd.cmd->forwardmove = 35.0f;
		//Interfaces::Globals->frametime *= (LocalPlayer.Entity->GetVelocity().Length2D()) / 1.f;
	...
	float inlatency = Interfaces::Engine->GetNetChannelInfo()->GetLatency(FLOW_INCOMING);
	...
	if (flDelta >= -0.1f)
	{
		if (flDelta < 0.03f)
		{
			if (flDelta >= -0.03f)
			{
				flNextGuessLowerBodyUPdateTime = (flServerTime - inlatency) + 1.1f;
			}
	...
	{
		float flGuessDelta = flNextGuessLowerBodyUPdateTime - flServerTime;

		DWORD cl = GetServerClientEntity(1);
		CBaseEntity *pEnt = ServerClientToEntity(cl);
		float nextupdatetime = pEnt->GetNextLowerBodyyawUpdateTimeServer();
		auto clientstae = LocalPlayer.Entity->GetPlayerAnimState();
		DWORD adr = (DWORD)&clientstae->m_flPitch - (DWORD)clientstae;

		if (fabsf(flGuessDelta) < 0.1f)
		{
			if (flGuessDelta >= -0.005f)
			{
				flNextGuessLowerBodyUPdateTime = (gLagCompensation.GetEstimatedServerTime() - inlatency) + 1.1f;
			}
