...
public:
	...
	COBB(void) {};
	COBB(const Vector& bbMin, const Vector& bbMax, matrix3x4_t *matrix, int ihitbox, int ihitgroup) { vecbbMin = bbMin; vecbbMax = bbMax; boneMatrix = matrix; hitbox = ihitbox, hitgroup = ihitgroup; };
};
...
public:
	...
	CSphere(void) {};
	CSphere(const Vector& vecCenter, float flRadius, int hitbox, int hitgroup) { m_vecCenter = vecCenter; m_flRadius = flRadius; Hitbox = hitbox; Hitgroup = hitgroup; };
...
void SetupCapsule(const Vector& vecMin, const Vector& vecMax, float flRadius, int hitbox, int hitgroup, std::vector<CSphere>&m_cSpheres);
