...
void OnResolvePitchPressed()
{
	if (pHighlightedPlayer)
	{
		...
		pHighlightedPlayer->PersistentData.ResolvePitchMode = (ResolvePitchModes)ResolvePitchDrop.index;
	}
	else
	{
...
void OnResolveYawPressed()
{
	if (pHighlightedPlayer)
	{
		...
		pHighlightedPlayer->PersistentData.ResolveYawMode = (ResolveYawModes)ResolveYawDrop.index;
	}
	else
	{
		ClearHighlightedPlayer();
...
{
	pCAPlayer->PersistentData.ResolvePitchMode = ResolvePitchModes::AutomaticPitch;
	pCAPlayer->PersistentData.ResolveYawMode = ResolveYawModes::AutomaticYaw;
	ResolverChk.Checked = true;
}

void UpdateResolverValues()
{
	ResolvePitchDrop.index = pHighlightedPlayer->PersistentData.ResolvePitchMode;
	ResolveYawDrop.index = pHighlightedPlayer->PersistentData.ResolveYawMode;
	DontTargetHeadNeckChk.Checked = pHighlightedPlayer->PersistentData.DontShootAtHeadNeck;
	ResolvePitchTxt.flValue = pHighlightedPlayer->PersistentData.flPitch;
	sprintf(ResolvePitchTxt.Value, "%f", pHighlightedPlayer->PersistentData.flPitch);
	ResolveYawTxt.flValue = pHighlightedPlayer->PersistentData.flYaw;
	sprintf(ResolveYawTxt.Value, "%f", pHighlightedPlayer->PersistentData.flYaw);
	PriorityTargetChk.Checked = pHighlightedPlayer->PersistentData.PriorityTarget;
	IgnoreAutoResolveChk.Checked = pHighlightedPlayer->PersistentData.DoNotResolve;
}
...
void OnResolvePitch()
{
	if (pHighlightedPlayer)
	{
		pHighlightedPlayer->PersistentData.flPitch = ResolvePitchTxt.flValue;
	}
}
...
void OnResolveYaw()
{
	if (pHighlightedPlayer)
	{
		pHighlightedPlayer->PersistentData.flYaw = ResolveYawTxt.flValue;
	}
}
...
void OnDontTargetHeadNeckClicked()
{
	if (pHighlightedPlayer)
	{
		pHighlightedPlayer->PersistentData.DontShootAtHeadNeck = DontTargetHeadNeckChk.Checked;
	}
	else
	{
		DontTargetHeadNeckChk.Checked = false;
...
void OnIgnoreClicked()
{
	if (pHighlightedPlayer)
	{
		pHighlightedPlayer->PersistentData.DoNotResolve = IgnoreAutoResolveChk.Checked;
		if (IgnoreAutoResolveChk.Checked)
		{
			pHighlightedPlayer->PersistentData.ResolveYawMode = ResolveYawModes::NoYaw;
			pHighlightedPlayer->PersistentData.ResolvePitchMode = ResolvePitchModes::NoPitch;
		}
	}
	else
	{
...
void OnPriorityClicked()
{
	if (pHighlightedPlayer)
	{
		pHighlightedPlayer->PersistentData.PriorityTarget = PriorityTargetChk.Checked;
	}
	else
	{
		PriorityTargetChk.Checked = false;
...
void RunVisualHacks_DXHook()
{
	...
	LogMessage("RunVisualHacks_DXHook\n");
#endif
...
#ifdef _DEBUG
	//CBaseCombatWeapon *pWeapon = LocalEntity->GetWeapon();
	//if (pWeapon)
	//{
		//char tst[42];
	...
	if (NumPlayers)
	{
		for (int i = 0; i <= MaxEntities; i++)
		{
			...
			//GLOW ESP
					...
					{
						DrawESP(LocalEntity, pBaseEntity, pCAPlayer, IsLockedOn);
					}
				}
			}
		}
	...
	//pSprite->End();
#ifdef PRINT_FUNCTION_NAMES
	LogMessage("Finished Visuals DX Hook\n");
#endif
}
...
void FillCustomPlayerStruct(CustomPlayer* const pCAPlayer, CBaseEntity*const pBaseEntity, int i)
{
	...
	if (pBaseEntity && pBaseEntity->IsPlayer())
	{
...
#endif
			Personalize* pPersistentData = &pCAPlayer->PersistentData;
			memset(&pCAPlayer->PersistentData, 0, (DWORD)&pPersistentData->m_PlayerLowerBodyRecords - (DWORD)pPersistentData); //hard coded for now.. You cannot memset a container or it will crash when you go to access it
		...
		{
			if (!pCAPlayer->PersistentData.DoNotResolve && pCAPlayer->PersistentData.ResolvePitchMode == NoPitch && pCAPlayer->PersistentData.ResolveYawMode == NoYaw)
			{
	...
	else
	{
		...
		Personalize TemporaryPersistentBuffer;
...
#endif
			TemporaryPersistentBuffer = pCAPlayer->PersistentData;
			char steamid[33];
			memcpy(steamid, pCAPlayer->steamid, 33);
			memset(pCAPlayer, 0, (DWORD)&pCAPlayer->PersistentData.m_PlayerLowerBodyRecords - (DWORD)pCAPlayer); //hard coded for now.. You cannot memset a container or it will crash when you go to access it
			pCAPlayer->PersistentData = TemporaryPersistentBuffer;
