...
void ClearAllPlayers()
{
	...
	for (int i = 0; i < 65; i++)
	{
		...
		{
#if 1
			if (pCPlayer->bHookedBaseEntity)
			{
				DeleteEntityHook(pCPlayer->HookedBaseEntity->hook);
				delete pCPlayer->HookedBaseEntity;
				pCPlayer->HookedBaseEntity = nullptr;
				pCPlayer->bHookedBaseEntity = false;
			}
#endif

			memset(pCPlayer, 0, (DWORD)&pCPlayer->PersistentData.m_PlayerLowerBodyRecords - (DWORD)pCPlayer); //hard coded for now.. You cannot memset a container or it will crash when you go to access it
...
StoredNetvars* CustomPlayer::GetBestHistoryTick()
{
	if (!IsBreakingLagCompensation)
	{
		bool bOldestHeadTickHadRealAngles = false;
		bool bOldestBodyTickHadRealAngles = false;
		StoredNetvars *pOldestHistoryTickWhereHeadWasFound = nullptr;
		StoredNetvars *pOldestHistoryTickWhereBodyWasFound = nullptr;

		for (auto& record : PersistentData.m_PlayerRecords)
		{
			if (!record->IsValidForBacktracking || !gLagCompensation.bTickIsValid(record->tickcount))
				break;

			if (record->bRanHitScan)
			{
				bool bHasRealAngles = record->LowerBodyUpdated || record->PitchingUp || record->IsRealTick;

				if (record->bHeadIsVisible)
				{
					//Try to find the oldest tick that had the real eye angles known
					if (!pOldestHistoryTickWhereHeadWasFound || bHasRealAngles)
					{
						if (bHasRealAngles)
							bOldestHeadTickHadRealAngles = true;
						pOldestHistoryTickWhereHeadWasFound = record;
					}
				}
				if (record->bBodyIsVisible)
				{
					//Try to find the oldest tick that had the real eye angles known
					if (!pOldestHistoryTickWhereBodyWasFound || bHasRealAngles)
					{
						if (bHasRealAngles)
							bOldestBodyTickHadRealAngles = true;
						pOldestHistoryTickWhereBodyWasFound = record;
					}
				}
			}
		}

		if (pOldestHistoryTickWhereHeadWasFound)
		{
			if (bOldestHeadTickHadRealAngles || (!pOldestHistoryTickWhereBodyWasFound || !bOldestBodyTickHadRealAngles))
				return pOldestHistoryTickWhereHeadWasFound;
		}

		return pOldestHistoryTickWhereBodyWasFound;
	}
