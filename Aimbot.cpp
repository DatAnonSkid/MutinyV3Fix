...
bool FindMultiPointVisible(int Bone, bool OnlyAutoWall, Vector &TargetBonePos, Vector &LocalEyePos, trace_t &tr, CBaseEntity* pPlayer, bool bforcehitgroup, int forcehitgroup, float& dmg, QAngle *localangles, bool HittingTargetFPS)
{
	...
	if (!AimbotMultiPointChk.Checked || (AimbotMultiPointPrioritiesOnlyChk.Checked && !AllPlayers[pPlayer->index].PersistentData.PriorityTarget))
	{
...
bool FindNewAimbotTarget(QAngle *localangles, CBaseCombatWeapon *weapon, Vector& BestBonePos, bool& RanHitChance, bool& CanTraceLineToATarget)
{
	...
	AngleVectors(*localangles, &vecDir);
	VectorNormalizeFast(vecDir);
	WeaponInfo_t* data = weapon ? weapon->GetCSWpnData() : nullptr;
	float flMaxTraceRange = data ? data->flRange : 8192.0f;
	...
	if (pAimingAt && MTargetting.IsPlayerAValidTarget(pAimingAt))
	{
		...
		//Shit fix for now untill hitchance + RCS is implemented correctly
		QAngle punch = angZero;
		if (IsAllowedToNoRecoil(weapon) && weapon)
		{
			QAngle vecPunch = LocalPlayer.Entity->GetPunch();
			punch.x = (vecPunch.x * 2.0f) * (NoRecoilYPercentTxt.flValue * 0.01f);
			punch.y = (vecPunch.y * 2.0f) * (NoRecoilXPercentTxt.flValue * 0.01f);
		}

		if (pSilentChk.Checked || gAutoBone.Run_AutoBone(weapon, pCPlayer, &(*localangles - punch), LocalEyePos, BestBone, BestBonePos, RanHitChance, CanTraceLineToATarget))
		{
	...
	for (LastCheckedIndex; LastCheckedIndex < maxcheckindex; LastCheckedIndex++)
	{
		...
		if (pCPlayer && pCPlayer->IsTarget && pCPlayer->LifeState == LIFE_ALIVE)
		{
			...
			for (int b = 0; b < (sizeof(HITBOXES_ALL_BASIC) / sizeof(HITBOXES_ALL_BASIC[0]) ); b++)
			{
				...
				pTPlayer->BonePos = pPlayer->GetBonePosition(hitbox, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
				RanHitChance = false;		
				...
				if (AllowAnyFOV || FOV <= AimbotFOVTxt.flValue)
				{
					...
					if (gAutoBone.Run_AutoBone(weapon, pCPlayer, &(*localangles - punch), LocalEyePos, BestBone, pTPlayer->BonePos, RanHitChance, LineHitThisTarget))
					{
						...
						{
							if (!pCPlayer->PersistentData.PriorityTarget && ExistingPlayer && TPlayers[IndexOfClosest].PriorityTarget)
							{
							...
							pTPlayer->PriorityTarget = pCPlayer->PersistentData.PriorityTarget;
...
bool AimToTheTarget(CBaseCombatWeapon* weapon, int buttons, QAngle &angles, QAngle* LastAngle, Vector LocalEyePos, Vector& TargetBonePos, CustomPlayer* pCPlayer, bool& AimbotModifiedAngles, int PressingTriggerbotKey )
{
	...
	StoredNetvars *CurrentTick = pCPlayer->PersistentData.m_PlayerRecords.size() ? pCPlayer->PersistentData.m_PlayerRecords.front() : nullptr;;

	if (ResolverChk.Checked && ( (pCPlayer->TicksChoked > 0 && !pCPlayer->bBacktracked && CurrentTick && CurrentTick->FireAtPelvis && (Interfaces::Globals->curtime - pCPlayer->TimeLowerBodyUpdated) < LowerBodyWaitTimeBeforeFiringTxt.flValue)
		|| (pCPlayer->PersistentData.ShouldResolve() && DontShootAfterXMissesTxt.iValue && pCPlayer->PersistentData.ShotsMissed > DontShootAfterXMissesTxt.iValue)
		 ))
...
bool Aimbot(QAngle& angles, DWORD* FramePointer, int &buttons, bool& AimbotModifiedAngles, QAngle LastAngle, CBaseCombatWeapon *weapon, int PressingTriggerbotKey, bool& RanHitChance)
{
	...
	if (LastTargetIndex != INVALID_PLAYER)
	{
		...
		else		
		{
			...
			if (gAutoBone.Run_AutoBone(weapon, pCPlayer, &(angles - punch), LocalEyePos, BestBone, BestBonePos, RanHitChance, CanTraceLineToATarget))
			{
				LastBoneChecked = BestBone;
