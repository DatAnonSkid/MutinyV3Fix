...
void AdjustTickCountForCmd(CBaseEntity *pPlayerFiredAt, bool bLocalPlayerIsFiring)
{
	...
	if (pPlayerFiredAt)
	{
		...
		if (pCPlayer->Backtracked)
		{
			...
			{
				int simtick = TIME_TO_TICKS(pCPlayer->CurrentSimulationTime);
				...
				{
					WriteInt((uintptr_t)&CurrentUserCmd->tick_count, ReadInt((uintptr_t)&CurrentUserCmd->tick_count) + lerpTicks);
				}
			}
			else
			{
		...
		{
			int targettick = TIME_TO_TICKS(pCPlayer->CurrentSimulationTime);
