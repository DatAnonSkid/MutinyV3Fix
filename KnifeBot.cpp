...
CBaseEntity* FindKnifeTarget(QAngle *LocalEyeAngles, Vector &TargetBonePos)
{
	...
	for (int i = 0; i < NumStreamedPlayers; i++)
	{
		...
		if (!pCPlayer->Dormant && pCPlayer->IsTarget && pCPlayer->LifeState == LIFE_ALIVE)
		{
			...
			TargetBonePos = pPlayer->GetBonePosition(LastBoneChecked, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
			...
			for (int k = 0; k < count; k++)
			{
				...
				TestPos = pPlayer->GetBonePosition(Bone, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
