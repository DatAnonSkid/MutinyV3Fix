...
bool Targetting::IsHitgroupATarget(int hitgroup, CBaseEntity* pEntity)
{
	...
	if (pEntity->IsPlayer())
	{
		...
		if (!pCPlayer->Dormant)
		{
			if (pCPlayer->PersistentData.DontShootAtHeadNeck && !pCPlayer->FireAtPelvis)
				HitHead = false;
		}
	}
...
CBaseEntity* Targetting::GetPlayerClosestToCrosshair(float MaxFOV)
{
	...
	for (int i = 0; i < NumStreamedPlayers; i++)
	{
		...
		if (pCPlayer && pCPlayer->IsTarget && pCPlayer->LifeState == LIFE_ALIVE)
		{
			...
			{
				pTPlayer->BonePos = pPlayer->GetBonePosition(HITBOXES_ALL_BASIC[b], Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
				...
				if (AllowAnyFOV || FOV <= MaxFOV)
				{
					...
					{
						if (!pCPlayer->PersistentData.PriorityTarget && IndexOfClosest != INVALID_PLAYER && TPlayers[IndexOfClosest].PriorityTarget)
...
void Targetting::AimStepBackToCrosshair(bool& DontSetViewAngles)
{
	...
	//FIXME
	float IncrementAngle = LegitAimStepChk.Checked ? 25.0f : (AimbotSpeedTxt.flValue < 25.0f ? 25.0f : AimbotSpeedTxt.flValue); // SpinDegPerTickTxt.flValue; //(LegitAimStepChk.Checked && SilentAimChk.Checked) ? (AimbotSpeedTxt.flValue * 2.5f + (SORandomFloat(0.2f, 0.3f) / deg)) : 42.5f;
