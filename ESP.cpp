...
void SkeltonEsp(CustomPlayer* player)
{
	...
	for(int i = 0; i < 10; i++)
	{
		...
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_HEAD, Interfaces::Globals->curtime, false, false, nullptr), W2S) == TRUE)
			data.head = Vector2D(W2S.x, W2S.y);
		else return;

		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_NECK, time, false, false, nullptr), W2S) == TRUE)
			data.neck = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_CHEST, time, false, false, nullptr), W2S) == TRUE)
			data.chest = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_PELVIS, time, false, false, nullptr), W2S) == TRUE)
			data.pelvis = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_LEFT_UPPER_ARM, time, false, false, nullptr), W2S) == TRUE)
			data.leftShoulder = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_RIGHT_UPPER_ARM, time, false, false, nullptr), W2S) == TRUE)
			data.rightShoulder = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_LEFT_FOREARM, time, false, false, nullptr), W2S) == TRUE)
			data.leftElbow = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_RIGHT_FOREARM, time, false, false, nullptr), W2S) == TRUE)
			data.rightElbow = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_LEFT_HAND, time, false, false, nullptr), W2S) == TRUE)
			data.leftHand = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_RIGHT_HAND, time, false, false, nullptr), W2S) == TRUE)
			data.rightHand = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_LEFT_CALF, time, false, false, nullptr), W2S) == TRUE)
			data.leftKnee = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_RIGHT_CALF, time, false, false, nullptr), W2S) == TRUE)
			data.rightKnee = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_LEFT_FOOT, time, false, false, nullptr), W2S) == TRUE)
			data.leftFoot = Vector2D(W2S.x, W2S.y);
		if (WorldToScreenCapped(player->BaseEntity->GetBonePosition(HITBOX_RIGHT_FOOT, time, false, false, nullptr), W2S) == TRUE)
	...
	if (WorldToScreenCapped(Origin, W2S) == TRUE && WorldToScreenCapped(HeadOrigin, W2S_TargetPos) == TRUE)
	{
		if (Entity->GetAlive())
		{
			...
			if (DrawResolveModeChk.Checked)
			{
				...
				switch (player->PersistentData.LastResolveModeUsed)
				{
					case NoYaw:
				...
				if (clclass)
				{
					...
					case _CPlantedC4:
						...
						if (flTimeToExplosion > -5.0f)
						{
							...
							int healthafterexplosion = pEntityToDrawHealth->index > 64 ? 0 : (int)(max((int)pEntityToDrawHealth->GetHealth() - max((int)ceilf(CSGO_Armor(flDamage, pEntityToDrawHealth->GetArmor())), 0), 0));
...
void RunAimESP()
{
	...
	for (int i = 0; i < NumStreamedPlayers; i++)
	{
		...
		if (pCPlayer->Active && !pCPlayer->IsLocalPlayer && !pCPlayer->Dormant && pCPlayer->IsTarget && pCPlayer->LifeState == LIFE_ALIVE && pCPlayer->pBacktrackedTick)
		{
			CBaseEntity *pEnt = pCPlayer->BaseEntity;
			pEnt->CacheBones(Interfaces::Globals->curtime, false, pCPlayer->pBacktrackedTick);
			//memcpy(&pCPlayer->CachedBoneMatrixes, &pCPlayer->pBacktrackedTick->CachedBoneMatrixes, sizeof(matrix3x4_t) * MAXSTUDIOBONES);
			...
			if (tr.DidHit())
			{
				Vector actualheadpos = pEnt->GetBonePosition(HITBOX_HEAD, Interfaces::Globals->curtime, false, true, pCPlayer->pBacktrackedTick);
