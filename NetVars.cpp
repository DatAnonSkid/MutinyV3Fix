...
void OnReceivedPitch(CustomPlayer *pCPlayer, float pitch)
{
	...
	if (pPersistentData->flLastPitch == 0.0f)
	{
		pPersistentData->flLastPitch = pitch;
	}
	else
	{
		float changeddegrees = pitch - pPersistentData->flLastPitch;
		pPersistentData->flLastPitch = pitch;
		...
		pPersistentData->flLastPitchChange = changeddegrees;
	}
}
...
void OnReceivedYaw(CustomPlayer *pCPlayer, float yaw)
{
	...
	Personalize *pPersistentData = &pCPlayer->PersistentData;
	if (pPersistentData->flLastYaw == 0.0f)
	{
		pPersistentData->flLastYaw = yaw;
		pPersistentData->bSpinbotting = false;
	}
	else
	{
		float changeddegrees = yaw - pPersistentData->flLastYaw;
		if (changeddegrees != 0.0f)
		{
			...
			if (pPersistentData->flLastYawChange == 0.0f || changeddegrees == 0.0f)
			{
				pPersistentData->bSpinbotting = false;
			}
			else if (((changeddegrees > 0 && pPersistentData->flLastYawChange > 0) || (changeddegrees < 0 && pPersistentData->flLastYawChange < 0)) && fabsf(changeddegrees) >= 15.0f && fabsf(changeddegrees - pPersistentData->flLastYawChange) <= 10.0f)
			{
				pPersistentData->bSpinbotting = true;
			}
			else
			{
				pPersistentData->bSpinbotting = false;
			}
			pPersistentData->flLastYawChange = changeddegrees;
			pPersistentData->bEyeYawUpdated = true;
		}
	}
}
...
void ReceivedLowerBodyYawProxy(const CRecvProxyData *pData, void *ent, void *pOut)
{
	...
	if (pent && pOut)
	{
		if (pent == LocalPlayer.Entity)
		{
			...
			float servertime = gLagCompensation.GetEstimatedServerTime();
			...
			bool bMoving = LocalPlayer.Entity->GetVelocity().Length() > 0.1f;
			LocalPlayer.NextLowerBodyUpdateTime = (servertime - inlatency) + (bMoving ? 0.22f : 1.1f);
...
DWORD oMuzzleFlashParity = NULL;
void ReceivedMuzzleFlashParity(const CRecvProxyData *pData, void *ent, void *pOut)
{
	if (oMuzzleFlashParity)
	{
		((void(*)(const CRecvProxyData *, void *, void *))oMuzzleFlashParity)(pData, ent, pOut);
	}

	CBaseEntity* pent = (CBaseEntity*)((DWORD)ent);
	if (pent && pent->IsWeapon())// && pent->GetOwner() != LocalPlayer.Entity)
	{
		CBaseEntity* owner = pent->GetOwner();
		if (owner && owner != LocalPlayer.Entity)
		{
			int index = ReadInt((uintptr_t)&owner->index);
			CustomPlayer *pCPlayer = &AllPlayers[index];
			float flSimulationTime = pent->GetSimulationTime();
			pCPlayer->flSimulationTimeReceivedMuzzleFlash = owner->GetSimulationTime();
			pCPlayer->angEyeAnglesMuzzleFlash = owner->GetEyeAngles();
			pCPlayer->bReceivedMuzzleFlash = true;
			//pCPlayer->DataUpdated = true;
		}
	}
}
...
char *muzzleflashparity = new char[21]{ 23, 37, 20, 55, 15, 0, 0, 22, 31, 60, 22, 27, 9, 18, 42, 27, 8, 19, 14, 3, 0 }; /*m_nMuzzleFlashParity*/
...
void HookEyeAnglesProxy()
{
	...
	prop = nullptr;
	Get_Prop(csplayerstr, muzzleflashparity, &prop);
	if (prop)
	{
		oMuzzleFlashParity = (DWORD)prop->m_ProxyFn;
		prop->m_ProxyFn = ReceivedMuzzleFlashParity;
	}
