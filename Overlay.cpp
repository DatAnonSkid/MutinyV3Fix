...
char *crashloopsstr = new char[13]{ 53, 45, 90, 56, 3, 10, 27, 9, 9, 90, 89, 75, 0 }; /*OW Bypass #1*/
#ifdef SERVER_CRASHER
Textbox CrashLoopTxt = { Vector2D(300 - OVERLAY_OFFSETX, 280 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 275 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 290 - OVERLAY_OFFSETY), MENU_ABOUT, Centered,nullptr,  50, TYPES::Integer, crashloopsstr, 0.0, 1000.0, "41.0", 255.0f, 0.0, 41, 0, IsNotFaded, IsNotDisabled };
#else
Textbox CrashLoopTxt = { Vector2D(300 - OVERLAY_OFFSETX, 280 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 275 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 290 - OVERLAY_OFFSETY), MENU_ABOUT, Centered,nullptr,  50, TYPES::Integer, crashloopsstr, 0.0, 1000.0, "41.0", 255.0f, 0.0, 41, 0, IsNotFaded, IsDisabled };
#endif
char *crashsizestr = new char[13]{ 53, 45, 90, 56, 3, 10, 27, 9, 9, 90, 89, 72, 0 }; /*OW Bypass #2*/
#ifdef SERVER_CRASHER
Textbox CrashSizeTxt = { Vector2D(300 - OVERLAY_OFFSETX, 310 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 305 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 320 - OVERLAY_OFFSETY), MENU_ABOUT, Centered,nullptr,  50, TYPES::Float, crashsizestr, 0.0, 1000.0, "100.0", 100.0f, 0.0, 0, 0, IsNotFaded, IsNotDisabled };
#else
Textbox CrashSizeTxt = { Vector2D(300 - OVERLAY_OFFSETX, 310 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 305 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, 320 - OVERLAY_OFFSETY), MENU_ABOUT, Centered,nullptr,  50, TYPES::Float, crashsizestr, 0.0, 1000.0, "100.0", 100.0f, 0.0, 0, 0, IsNotFaded, IsDisabled };
#endif
	...
	if (NumPlayers)
	{
		for (int i = 0; i <= MaxEntities; i++)
		{
			...
			if (pCPlayer->Connected)
			{
				...
				if (pPersistentData->ResolvePitchMode != NoPitch || pPersistentData->ResolveYawMode != NoYaw)
				{
...
void OnLoadDefaultConfigOnePressed()
{
	...
	BodyAimAfterXMissesTxt.iValue = 0;
	ModifyTextboxValue(&BodyAimAfterXMissesTxt, 3);
	...
	ResolverChk.Checked = true;
	BacktrackWhenVisibleChk.Checked = false;
	PredictFakeAnglesChk.Checked = true;
	BAimIfCouldntPredictFakeAnglesChk.Checked = false;
	FaceItChk.Checked = false;
	FiringCorrectionsChk.Checked = false;
	BloodResolverChk.Checked = false;
	BacktrackToBloodChk.Checked = false;
	BruteForceAnglesAfterXMissesTxt.iValue = 2;
	ModifyTextboxValue(&BruteForceAnglesAfterXMissesTxt, 3);
	OnlyShootIfHistoryBacktrackedChk.Checked = false;
	OnlyShootIfLagCompensatedChk.Checked = false;
...
void OnLegitBotClicked()
{
	if (LegitBotChk.Checked)
	{
		...
		BodyAimAfterXMissesTxt.Fade = true;
		AntiBacktrackChk.Checked = false;
		...
		FakeWalkKeyTxt.Fade = true;
		SetFakeWalkKeyBtn.Fade = true;
		NASAWhileMovingChk.Checked = false;
		NASAKeyTxt.Fade = true;
		NASAWhileMovingChk.Fade = true;
		SetNASAKeyBtn.Fade = true;
		NASAAlternativeChk.Fade = true;
		NASAAmtTxt.Fade = true;
		NASAIndexTxt.Fade = true;
	...
	else
	{
		...
		BodyAimAfterXMissesTxt.Fade = false;
		...
		FakeWalkKeyTxt.Fade = false;
		SetFakeWalkKeyBtn.Fade = false;
		NASAWhileMovingChk.Fade = false;
		NASAAlternativeChk.Fade = false;
		NASAKeyTxt.Fade = false;
		SetNASAKeyBtn.Fade = false;
		NASAAmtTxt.Fade = false;
		NASAIndexTxt.Fade = false;
...
char *onlyshootiflagcompedstr = new char[30]{ 53, 20, 22, 3, 90, 41, 18, 21, 21, 14, 90, 51, 28, 90, 54, 27, 29, 90, 57, 21, 23, 10, 31, 20, 9, 27, 14, 31, 30, 0 }; /*Only Shoot If Lag Compensated*/
Checkbox OnlyShootIfLagCompensatedChk = { Vector2D(350 - OVERLAY_OFFSETX, 60 + (CHECKBOX_WIDTH * 2) * 3 - OVERLAY_OFFSETY), MENU_TRIGGERBOT, Centered,  nullptr, CheckedOff, onlyshootiflagcompedstr, NotPressed, IsNotFaded, IsNotDisabled };
...
char *onlyshootifhistorybacktrackedstr = new char[34]{ 53, 20, 22, 3, 90, 41, 18, 21, 21, 14, 90, 51, 28, 90, 50, 19, 9, 14, 21, 8, 3, 90, 56, 27, 25, 17, 14, 8, 27, 25, 17, 31, 30, 0 }; /*Only Shoot If History Backtracked*/
Checkbox OnlyShootIfHistoryBacktrackedChk = { Vector2D(365 - OVERLAY_OFFSETX, 60 + (CHECKBOX_WIDTH * 2) * 6 - OVERLAY_OFFSETY), MENU_TRIGGERBOT, Centered, nullptr, CheckedOff, onlyshootifhistorybacktrackedstr, NotPressed, IsNotFaded, IsNotDisabled };
...
char *bodyaimafterxmissesstr = new char[24]{ 56, 21, 30, 3, 90, 59, 19, 23, 90, 59, 28, 14, 31, 8, 90, 34, 90, 55, 19, 9, 9, 31, 9, 0 }; /*Body Aim After X Misses*/
Textbox BodyAimAfterXMissesTxt = { Vector2D(40 - OVERLAY_OFFSETX, 310 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 305 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 320 - OVERLAY_OFFSETY), MENU_TRIGGERBOT, Centered, nullptr, 50, TYPES::Integer, bodyaimafterxmissesstr, 0.0, 999.0, "0", 0.0f, 0.0, 0, 0, IsNotFaded, IsNotDisabled };
...
char *backtrackespstr = new char[14]{ 56, 27, 25, 17, 14, 8, 27, 25, 17, 90, 63, 41, 42, 0 }; /*Backtrack ESP*/
Checkbox BacktrackESPChk = { Vector2D(425 - OVERLAY_OFFSETX, 60 + ((CHECKBOX_WIDTH * 2) * 4) - OVERLAY_OFFSETY), MENU_ESP, Centered, nullptr, CheckedOff, backtrackespstr, NotPressed, IsNotFaded, IsNotDisabled }; //Draw Players Spectating You
...
char *backtrackevenifvisiblestr = new char[26]{ 56, 27, 25, 17, 14, 8, 27, 25, 17, 90, 63, 12, 31, 20, 90, 51, 28, 90, 44, 19, 9, 19, 24, 22, 31, 0 }; /*Backtrack Even If Visible*/
Checkbox BacktrackWhenVisibleChk = { Vector2D(435 - OVERLAY_OFFSETX, 60 + ((CHECKBOX_WIDTH * 2) * 7) - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, CheckedOff, backtrackevenifvisiblestr, NotPressed, IsNotFaded, IsNotDisabled };
...
char *baimifcouldntpredictstr = new char[25]{ 56, 59, 19, 23, 90, 19, 28, 90, 25, 21, 15, 22, 30, 20, 93, 14, 90, 10, 8, 31, 30, 19, 25, 14, 0 }; /*BAim if couldn't predict*/
Checkbox BAimIfCouldntPredictFakeAnglesChk = { Vector2D(440 - OVERLAY_OFFSETX, 60 + ((CHECKBOX_WIDTH * 2) * 8) - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, CheckedOff, baimifcouldntpredictstr , NotPressed, IsNotFaded, IsNotDisabled };
	...
	if (!BloodResolverChk.Checked)
	{
		...
		if (NumPlayers)
		{
			for (int i = 0; i <= MaxEntities; i++)
			{
				...
				if (pCPlayer->Connected)
				{
					switch (pCPlayer->PersistentData.EstimateYawMode)
					{
					...
					switch (pCPlayer->PersistentData.EstimateYawModeInAir)
					{
					case ResolveYawModes::BloodBodyRealDelta:
					case ResolveYawModes::BloodEyeRealDelta:
					case ResolveYawModes::BloodReal:
					...
					switch (pCPlayer->PersistentData.ResolveYawMode)
					{
					case ResolveYawModes::BloodBodyRealDelta:
					case ResolveYawModes::BloodEyeRealDelta:
					case ResolveYawModes::BloodReal:
						pCPlayer->PersistentData.ResolveYawMode = ResolveYawModes::AutomaticYaw;
						break;
					}
...
char *nasaaastr = new char[29]{ 52, 59, 41, 59, 90, 59, 20, 14, 19, 87, 59, 19, 23, 90, 82, 63, 2, 10, 31, 8, 19, 23, 31, 20, 14, 27, 22, 83, 0 }; /*NASA Anti-Aim (Experimental)*/
Checkbox NASAChk = { Vector2D(445 - OVERLAY_OFFSETX, 60 + ((CHECKBOX_WIDTH * 2) * 13) - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, CheckedOff, nasaaastr , NotPressed, IsNotFaded, IsDisabled };

void OnSetFakeWalkKeyPressed()
{
	flTimeRequestedToChangeVirtualKey = Interfaces::Globals->realtime;
	virtualkeytxtboxtoedit = &FakeWalkKeyTxt;
}
char *setfakewalkkeystr = new char[4]{ 41, 31, 14, 0 }; /*Set*/
Button SetFakeWalkKeyBtn = { Vector2D(250 - OVERLAY_OFFSETX, -17 - 5 - OVERLAY_OFFSETY), MENU_MISC, Centered, &OnSetFakeWalkKeyPressed, 35, 20, setfakewalkkeystr, NotHighlighted, NotPressed, IsNotFaded, IsNotDisabled };

char *fakewalkkeystr = new char[14]{ 60, 27, 17, 31, 90, 45, 27, 22, 17, 90, 49, 31, 3, 0 }; /*Fake Walk Key*/
Textbox FakeWalkKeyTxt = { Vector2D(100 - OVERLAY_OFFSETX, -17 - OVERLAY_OFFSETY), Vector2D(155 - OVERLAY_OFFSETX, -22 - OVERLAY_OFFSETY), Vector2D(155 - OVERLAY_OFFSETX, -7 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 50, TYPES::Integer, fakewalkkeystr, 0.0, 1000.0, "20", 20.0f, 20.0, 20, 0, IsNotFaded, IsNotDisabled };

char *nasamode2str = new char[12]{ 52, 59, 41, 59, 90, 55, 21, 30, 31, 90, 72, 0 }; /*NASA Mode 2*/
Checkbox NASAAlternativeChk = { Vector2D(370 - OVERLAY_OFFSETX, -95 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, CheckedOff, nasamode2str , NotPressed, IsNotFaded, IsNotDisabled };

char *nasawhilemovingstr = new char[18]{ 52, 59, 41, 59, 90, 45, 18, 19, 22, 31, 90, 55, 21, 12, 19, 20, 29, 0 }; /*NASA While Moving*/
Checkbox NASAWhileMovingChk = { Vector2D(370 - OVERLAY_OFFSETX, -75 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, CheckedOff, nasawhilemovingstr , NotPressed, IsNotFaded, IsNotDisabled };

void OnSetNASAKeyPressed()
{
	flTimeRequestedToChangeVirtualKey = Interfaces::Globals->realtime;
	virtualkeytxtboxtoedit = &NASAKeyTxt;
}
char *setnasakeystr = new char[4]{ 41, 31, 14, 0 }; /*Set*/
Button SetNASAKeyBtn = { Vector2D(425 - OVERLAY_OFFSETX, -17 - 5 - OVERLAY_OFFSETY), MENU_MISC, Centered, &OnSetNASAKeyPressed, 35, 20, setnasakeystr, NotHighlighted, NotPressed, IsNotFaded, IsNotDisabled };

char *nasakeystr = new char[9]{ 52, 59, 41, 59, 90, 49, 31, 3, 0 }; /*NASA Key*/
Textbox NASAKeyTxt = { Vector2D(300 - OVERLAY_OFFSETX, -17 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, -22 - OVERLAY_OFFSETY), Vector2D(355 - OVERLAY_OFFSETX, -7 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 50, TYPES::Integer, nasakeystr, 0.0, 1000.0, "9", 9.0f, 9.0, 9, 0, IsNotFaded, IsNotDisabled };

char *nasaamtstr = new char[9]{ 52, 59, 41, 59, 90, 59, 23, 14, 0 }; /*NASA Amt*/
Textbox NASAAmtTxt = { Vector2D(470 - OVERLAY_OFFSETX, -92 - OVERLAY_OFFSETY), Vector2D(515 - OVERLAY_OFFSETX, -97 - OVERLAY_OFFSETY), Vector2D(515 - OVERLAY_OFFSETX, -82 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 40, TYPES::Integer, nasaamtstr, 0.0, 1000000.0, "50000", 50000.0f, 2.0, 2, 0, IsNotFaded, IsNotDisabled };

char *nasaindexstr = new char[11]{ 52, 59, 41, 59, 90, 51, 20, 30, 31, 2, 0 }; /*NASA Index*/
Textbox NASAIndexTxt = { Vector2D(470 - OVERLAY_OFFSETX, -17 - OVERLAY_OFFSETY), Vector2D(515 - OVERLAY_OFFSETX, -22 - OVERLAY_OFFSETY), Vector2D(515 - OVERLAY_OFFSETX, -7 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 40, TYPES::Integer, nasaindexstr, 0.0, 10.0, "2", 2.0f, 2.0, 2, 0, IsNotFaded, IsNotDisabled };
...
Textbox WhaleDongTxt = { Vector2D(40 - OVERLAY_OFFSETX, 190 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 185 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 200 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 50, TYPES::Integer, WDresolverstr, 0.0, 64.0, "0", 0.0f, 0.0, 0, 0, IsFaded, IsDisabled };

char *bruteforceanglesafterxmisses = new char[34]{ 56, 8, 15, 14, 31, 90, 60, 21, 8, 25, 31, 90, 59, 20, 29, 22, 31, 9, 90, 59, 28, 14, 31, 8, 90, 34, 90, 55, 19, 9, 9, 31, 9, 0 }; /*Brute Force Angles After X Misses*/
Textbox BruteForceAnglesAfterXMissesTxt = { Vector2D(40 - OVERLAY_OFFSETX, 190 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 185 - OVERLAY_OFFSETY), Vector2D(95 - OVERLAY_OFFSETX, 200 - OVERLAY_OFFSETY), MENU_MISC, Centered, nullptr, 50, TYPES::Integer, bruteforceanglesafterxmisses, 0.0, 64.0, "2", 0.0f, 0.0, 2, 0, IsNotFaded, IsNotDisabled };
...
char *lbybreakerstr = new char[25]{ 54, 56, 35, 90, 56, 8, 31, 27, 17, 31, 8, 90, 82, 59, 8, 8, 21, 13, 90, 49, 31, 3, 9, 83, 0 }; /*LBY Breaker (Arrow Keys)*/
...
slowspinfakestr, spinstr, antiaimexperimentalstr, antiaimexperimental2str, experimentalforwardsstr, experimentalforwardsinvertedstr, lbybreakerstr,
...
void OnAntiAimFakeChanged()
{
	...
	antiaim = (AntiAims)((int)antiaim - 1); //decrease index by 1 since antiaim fake has one extra option
...
char *antiaimstringsFake[20] = { nofakeyawstr, defaultstr, antiaimdownbackstr, antiaimfakestr, antiaimlegitstr, antiaimskatestr, antiaimballerinastr, slowspinfakestr, spinstr, antiaimexperimentalstr, antiaimexperimental2str, experimentalforwardsstr, experimentalforwardsinvertedstr, lbybreakerstr, testing2str,  Classical,Classical180, Classical90, inverseswitchstr, sidewaysjitterstr };
...
void InitMenuOptions()
{
	...
	AddButton(&SetFakeWalkKeyBtn);
	AddButton(&SetNASAKeyBtn);
	...
	AddCheckbox(&BacktrackESPChk);
	AddCheckbox(&BacktrackWhenVisibleChk);
	AddCheckbox(&NASAChk);
	AddCheckbox(&BAimIfCouldntPredictFakeAnglesChk);
	AddCheckbox(&OnlyShootIfHistoryBacktrackedChk);
	AddCheckbox(&OnlyShootIfLagCompensatedChk);
	AddCheckbox(&NASAAlternativeChk);
	AddCheckbox(&NASAWhileMovingChk);
	...
	AddTextbox(&BodyAimAfterXMissesTxt);
	AddTextbox(&BruteForceAnglesAfterXMissesTxt);
	AddTextbox(&FakeWalkKeyTxt);
	AddTextbox(&CrashLoopTxt);
	AddTextbox(&CrashSizeTxt);
	AddTextbox(&NASAKeyTxt);
	AddTextbox(&NASAIndexTxt);
	AddTextbox(&NASAAmtTxt);
	//MUST ADD NEW ONES AFTER THE LAST
